...
class Gaussian:
    ...
    def sample(self):
        da = array([random.gauss(0.0, 1.0) for x in range(self.N)])
...
class Game:
    ...
    def rebuild_samples(self, SamList, Gausses):
        ...
        #find maxlike,maxlikesa and gmaxlike
        for sa in SamList:
            if sa.like > maxlike:
                maxlike = sa.like
                maxlikesa = sa
        ...
        effsamp = 0
        for sa in SamList:
            sa.rellike = exp(sa.like - maxlike)
    ...
    def getcov(self, around):
        ...
        #     for j in range(N):
        #         if (i==j):
        #             der=(list(likes).pop(0)+list(likes).pop(0)-2*like0)/(delta[i]**2)
        #         else:
        #             der=(list(likes).pop(0)+list(likes).pop(0)-list(likes).pop(0)-list(likes).pop(0))/(4*delta[i]*delta[j])
        #         icov[i,j]=-der
        #         icov[j,i]=-der
