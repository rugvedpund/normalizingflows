...
parser = argparse.ArgumentParser(description="Normalizing Flow for 21cm maps")

#arguments for ForegroundModel
parser.add_argument('--fgFITS', type=str, required=False, default='ulsa.fits')
parser.add_argument('--freqsFITS', type=str, required=False, default='1 51') #e.g. '1 51' separated by space, used when creating foreground model
...
while done==False:
    ...
    print(kwargs)
    samples,t21_vs=nf.get_t21vs(flow.freqs,npoints,**kwargs)
...
for vs in ['A']:
    ...
    samples1d,t21_vs1d=nf.get_t21vs1d(flow.freqs,npoints,vs,**kwargs)
