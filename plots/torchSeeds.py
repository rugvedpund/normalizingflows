...
def format_constraints(truth, nseedmeans, tseedmeans):
    nseedmeans, tseedmeans = np.array(nseedmeans), np.array(tseedmeans)
    bias = np.mean(nseedmeans - truth)
    varbias = np.var(nseedmeans - truth)
    varsinf = np.var(tseedmeans)
    stdsinf = np.sqrt(varsinf)
    stdnoise = np.sqrt(np.abs(varbias - varsinf))
    ...
    return rf"${truth}\,$mK(truth) ${bias:+.1f}$(bias) $\pm {stdsinf:.1f}$(sinf) $\pm {stdnoise:.1f}$(noise)"
...
plt.plot([], [], "C0", label="SINF Seed Variation", lw=3.0, alpha=0.5)
plt.plot([], [], "C1", label="Noise Seed Variation", lw=3.0, alpha=0.5)
...
plt.savefig("torchSeeds3.pdf", bbox_inches="tight", dpi=300)
