...
def plot_weighted_boxplot(
    samples, likelihoods, ax, y_value, c="C0", hatch=None, box_width=0.1, **kwargs
):
...
chromaticerrbars = {"fmt": "s", "markersize": 5, "capsize": 4}
errbars95 = {"fmt": "", "capsize": 4, "xuplims": True, "markersize": 1}
dasnrpps = [1e7,1e8,1e9,1e10]
...
ax[0, 0].axvline(truth, **truthaxvlines)
ax[0, 1].axvline(truth, **truthaxvlines)
ax[1, 0].axvline(truth, **truthaxvlines)
ax[1, 1].axvline(truth, **truthaxvlines)

ax[0, 0].axvspan(0.01, 0.1*truth, color="k", alpha=0.1)
ax[0, 1].axvspan(0.01, 0.1*truth, color="k", alpha=0.1)
ax[1, 0].axvspan(0.01, 0.1*truth, color="k", alpha=0.1)
ax[1, 1].axvspan(0.01, 0.1*truth, color="k", alpha=0.1)
...
ax[1, 0].set_xlabel(r"$A$ [K]")
ax[1, 1].set_xlabel(r"$A$ [K]")

ax[1, 0].set_xlim(0.1, 40)
ax[1, 1].set_xlim(0.1, 40)

ax[1, 0].set_xscale("log")
ax[1, 1].set_xscale("log")

ax[1, 0].set_ylabel("Signal-to-Noise Ratio", fontsize="small", labelpad=10)
ax[1, 1].set_ylabel("Signal-to-Noise Ratio", fontsize="small", labelpad=10)

ax[0, 0].set_title("Cosmic Dawn", fontsize="small")
ax[0, 1].set_title("Dark Ages", fontsize="small")
fig.suptitle("CMB Monopole Likelihood")
